...
class OpenUSDConan(ConanFile):
    ...
    default_options = {
        ...
        "build_usdview": True,
        # FIXME:
#         /root/.conan2/p/b/openu5e99139527369/b/src/pxr/imaging/plugin/hioOiio/oiioImage.cpp:44:10: fatal error: OpenImageIO/imagebuf.h: No such file or directory
...
#    32 | #include <embree3/rtcore.h>
        ...
        # FIXME:
#         /root/.conan2/p/b/openub65bab647f058/b/src/pxr/imaging/hdSt/ptexMipmapTextureLoader.h:29:10: fatal error: Ptexture.h: No such file or directory
#    29 | #include <Ptexture.h>
        ...
        # FIXME:
#         /root/.conan2/p/b/openu0d9487be679c5/b/src/pxr/usd/plugin/usdAbc/alembicUtil.h:38:10: fatal error: Alembic/Abc/ICompoundProperty.h: No such file or directory
#    38 | #include <Alembic/Abc/ICompoundProperty.h>

        "build_alembic_plugin": False,
        "enable_hdf5_support": True,
        # FIXME:
#         /root/.conan2/p/b/openub6914422cb70b/b/src/pxr/usd/plugin/usdDraco/attributeDescriptor.h:34:10: fatal error: draco/attributes/geometry_attribute.h: No such file or directory
#    34 | #include <draco/attributes/geometry_attribute.h>
    ...
    def config_options(self):
        ...
        self.options.build_gpu_support = self.options.enable_gl_support or self.options.enable_metal_support or self.options.enable_vulkan_support
    ...
    def requirements(self):
        ...
        # TODO: rework TBB dependency once https://github.com/PixarAnimationStudios/OpenUSD/issues/1471 fixed
        # FIXME: failed to build on Fedora 40:
        #         ../../include/tbb/task.h:289:20: erreur: la déclaration de « tbb::task& tbb::internal::task_prefix::task() » change la signification de « task » [-Wchanges-meaning]
        #   289 |         tbb::task& task() {return *reinterpret_cast<tbb::task*>(this+1);}
        # WORKAROUND: build using a docker image:
    ...
    def generate(self):
        ...
        # FIXME: got tbb linking issue with binaries
        tc.variables["PXR_BUILD_USD_TOOLS"] = self.options.build_usd_tools
        
        tc.variables["OPENSUBDIV_LIBRARIES"] = "opensubdiv::opensubdiv"
        tc.variables["OPENSUBDIV_INCLUDE_DIR"] = self.dependencies['opensubdiv'].cpp_info.includedirs
        
        tc.variables["TBB_tbb_LIBRARY"] = "TBB::tbb"
        tc.variables["OPENVDB_LIBRARY"] = "OpenVDB::openvdb"
        # FIXME: get following error with openvdb enabled:
#         CMake Error in pxr/imaging/hioOpenVDB/CMakeLists.txt:
#   Target "hioOpenVDB" INTERFACE_INCLUDE_DIRECTORIES property contains path:

#     "/root/.conan2/p/b/openuf3d9cdbb860ae/b/src/pxr/imaging/hioOpenVDB/['/root/.conan2/p/b/openv86edcb8bedbc9/p/include']"

#   which is prefixed in the source directory.
        # tc.variables["OPENVDB_INCLUDE_DIR"] = self.dependencies['openvdb'].cpp_info.includedirs
        # self.output.info("self.deps_cpp_info['openvdb'].includedirs")
        # self.output.info(str(self.dependencies['openvdb'].cpp_info.includedirs))
    ...
    def _patch_sources(self):
        ...
        pxrImagingpxOsdCMakeListsToUpdate = os.path.join(self.source_folder, "pxr/imaging/pxOsd/CMakeLists.txt")
        replace_in_file(self, pxrImagingpxOsdCMakeListsToUpdate, "set(PXR_PACKAGE pxOsd)", 
                        """
                        set(PXR_PACKAGE pxOsd)
                        message(STATUS "OPENSUBDIV_LIBRARIES: ${OPENSUBDIV_LIBRARIES}")
                        """)
        
        cmakeMacrosPrivateCmake = os.path.join(self.source_folder, "cmake/macros/Private.cmake")
        replace_in_file(self, cmakeMacrosPrivateCmake, "        target_link_libraries(${NAME}", 
                        """
                        message(STATUS "NAME: ${NAME}")
                        message(STATUS "internal: ${internal}")
                        message(STATUS "external: ${external}")
                        message(STATUS "PXR_MALLOC_LIBRARY: ${PXR_MALLOC_LIBRARY}")
                        message(STATUS "PXR_THREAD_LIBS: ${PXR_THREAD_LIBS}")
    ...
    def package_info(self):
        ...
        self.cpp_info.components["usd_usdGeom"].requires = ["usd_arch", "usd_kind", "usd_pcp", "usd_sdf", "usd_ar", "usd_plug", "usd_tf", "usd_trace", "usd_vt",  "usd_work", "onetbb::libtbb"]
